...
CINELINK_WEBDAV_BLOCK_SIZE=1048576
```
...
新生成的 STRM 会优先使用 `CINELINK_PLAY_PUBLIC_URL` 的播放代理地址，例如 `http://192.168.1.10:8000/play/quark/...`，它会把播放器的 Range 请求直接转发给网盘 CDN。
...
`CINELINK_WEBDAV_BLOCK_SIZE` 控制内置 WebDAV 每次向播放器发送的数据块大小，默认 1MB，适合视频播放。
